  ...
  return <section className="connection-detail workspace-detail">
    ...
    <div className="workspace-tab-content">
      ...
      {activeTab === 'general' ? <>
        ...
        <section className="connection-section workspace-general-info"><div className="connection-section-heading"><h3>Información general</h3><button type="button" className="client-button-secondary" onClick={() => setIsEditingName((value) => !value)}><Pencil size={15} aria-hidden="true" /> Editar nombre</button></div><dl className="connection-information-list"><div><dt>Cliente</dt><dd>{connection.client?.name || 'No disponible'}</dd></div><div><dt>Canal</dt><dd>{connection.channel.displayName}</dd></div><div><dt>Provider</dt><dd>{connection.provider.displayName}</dd></div>{isInstagram ? <div><dt>Provider account ID</dt><dd>{connection.providerAccount?.providerAccountId || 'No vinculada'}</dd></div> : null}<div><dt>Estado</dt><dd><StatusBadge tone={headerState.tone}>{headerState.label}</StatusBadge></dd></div><div><dt>Última actividad</dt><dd>{dateTime(statusSummary?.lastActivityAt || connection.lastActivityAt)}</dd></div></dl></section>
        {isInstagram ? <><InstagramConnectionPanel connection={connection} onConnectionChange={(updated) => { setConnection(updated) }} /><OperationsDiagnostics connectionId={connection.id} providerId={connection.provider.id} onReconnect={reconnect} onRefreshConnection={refreshWorkspace} onManageWebhooks={() => navigate(`/connections/${connection.id}/webhooks`)} /></> : <>
